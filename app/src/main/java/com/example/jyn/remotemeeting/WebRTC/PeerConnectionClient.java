...
public class PeerConnectionClient {
    ...
    private static final String TAG = "all_"+PeerConnectionClient.class.getSimpleName();
    ...
    //
    // 대기중인 원격 ICE 후보는 로컬 및 원격 설명을 모두 설정 한 후에만 사용됩니다.
    // 로컬 및 리모트의 양쪽 모두의 기술이 설정되면, 리모트 피어가 설정됩니다.
     ...
     * 피어 커넥션 파라미터 - DataChannelParameters
     */
    ...
    /**
     * Peer connection parameters.
     * 피어 커넥션 파라미터 - PeerConnectionParameters
     */
    ...
    public interface PeerConnectionEvents {
         ...
         * local Ice 후보가 생성되면 콜백이 시작됩니다.
         */
        void onIceCandidate(final IceCandidate candidate);

        /**
         * Callback fired once local ICE candidates are removed.
         * local ICE 후보가 제거되면 콜백이 시작됩니다.
         */
         ...
         * 연결이 닫히면 콜백이 시작됩니다 (IceConnectionState는 DISCONNECTED 임).
         */
    ...
    private PeerConnectionClient() {
        // Executor thread is started once in private sector and is used for all
        // peer connection API calls to ensure new peer connection factory is
        // created on the same thread as previously destroyed factory.
        //
        // Executor 스레드는 사설 섹터에서 한 번 시작되며,
        // 모든 피어 연결 API 호출에 사용되어
        // 새로운 피어 연결 팩토리가 이전에 파기된 팩토리와 동일한 스레드에 생성되도록합니다.
    ...
    private void createMediaConstraintsInternal() {
        ...
        // 기기에 카메라가 있는지 확인하고 그렇지 않은 경우 화상 통화를 사용 중지.
        if (videoCapturer == null) {
            Log.w(TAG, "No camera on device. Switch to audio only call.");
            videoCallEnabled = false;
        }
        // Create video constraints if video call is enabled.
        // 비디오 호출이 활성화 된 경우 비디오 제약 조건을 생성
        if (videoCallEnabled) {
            ...
            // 비디오 해상도를 지정하지 않으면 기본값
            if (videoWidth == 0 || videoHeight == 0) {
                videoWidth = Static.DEFAULT_VIDEO_WIDTH;
                videoHeight = Static.DEFAULT_VIDEO_HEIGHT;
            }

            // If fps is not specified, default to 30.
            // fps가 지정되지 않은 경우 기본값은 30.
            if (videoFps == 0) {
                videoFps = 30;
            }
    ...
    private void createPeerConnectionInternal(EglBase.Context renderEGLContext) {
        ...
        // NOTE: this must happen while |factory| is alive!
        //
        // 기본 WebRTC 추적 및 INFO libjingle 로깅을 설정.
        // 알림: 이 것은 |factory|가 살아있는 동안 반드시 발생해야한다.
        ...
        executor.execute(new Runnable() {
            ...
            public void run() {
                ...
                for (RtpParameters.Encoding encoding : parameters.encodings) {
                    ...
                    encoding.maxBitrateBps = maxBitrateKbps == null ? null : maxBitrateKbps * Static.BPS_IN_KBPS;
                    Log.d(TAG, "encoding.maxBitrateBps: "+String.valueOf(maxBitrateKbps * Static.BPS_IN_KBPS));
                }
    ...
    private static String setStartBitrate(
        ...
        // 이 코덱의 원격 SDP에 a=fmtp 문자열이 이미 있는지 확인하고 새 비트 전송률 매개 변수로 업데이트.
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            // 코덱에 해당 라인이없는 경우 새 a=fmtp 라인을 추가.
    ...
    private static String movePayloadTypesToFront(List<String> preferredPayloadTypes, String mLine) {
        ...
        // 미디어 설명 행의 형식은 다음과 같아야합니다. m = <media> <port> <proto> <fmt> ...
        ...
        // 페이로드 유형의 시작 부분으로 이동되는 "preferredPayloadTypes"로 라인을 재구성하십시오.
    ...
    private static String preferCodec(String sdpDescription, String codec, boolean isAudio) {
        ...
        // 페이로드 유형은 96-127 범위의 정수이지만 여기에는 문자열로 저장됨.
    ...
    private class PCObserver implements PeerConnection.Observer {
        ...
        public void onRenegotiationNeeded() {
            ...
            // 아무 것도 할 필요가 없습니다. AppRTC는 사전 합의 된 신호/협상 프로토콜을 따릅니다.
        }
    ...
    //
    // 구현 세부 사항 : SDP가 설정되면 원격 ICE 후보자를 추가 할뿐만 아니라,
    // offer 생성/신호 및 응답 설정을 처리합니다.
            ...
            executor.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    if (isInitiator) {
                        ...
                        // 피어 연결 제공을 위해 먼저 제안을 만들고 로컬 SDP를 가져온 다음 응답을받은 후 원격 SDP를 설정합니다.
                        ...
                        // 피어 연결에 응답하기 위해 우리는 원격 SDP를 설정 한 다음 응답을 작성하고 로컬 SDP를 설정합니다.
